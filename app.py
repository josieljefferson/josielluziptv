...
# NO RENDER, usar a porta da variável de ambiente PORT
# Se não existir, usar 8080 como fallback
PORT = int(os.environ.get("PORT", 8080))
HOST = "0.0.0.0"  # Importante: usar 0.0.0.0 para escutar em todas as interfaces

def server_url():
    """Retorna a URL base do servidor considerando HTTPS no Render"""
    # No Render, sempre usar https se estiver em produção
    if os.environ.get("RENDER"):
        # No Render, a URL é fornecida via RENDER_EXTERNAL_URL
        render_url = os.environ.get("RENDER_EXTERNAL_URL", "")
        if render_url:
            return render_url.rstrip("/")
    
    # Para desenvolvimento local ou outros ambientes
    base_url = request.host_url.rstrip("/") if request.host_url else f"http://localhost:{PORT}"
    
    # Se estiver no Render mas sem RENDER_EXTERNAL_URL, usar https
    if os.environ.get("RENDER") and not base_url.startswith("https"):
...
def redirect_to_https():
    """Redireciona para HTTPS no Render se necessário"""
    # No Render, o balanceador de carga usa HTTP internamente
    # Mas externamente queremos HTTPS
    if os.environ.get("RENDER"):
        # Verificar se a requisição veio via HTTP (não confiável no Render)
        # Em vez disso, sempre gerar URLs com HTTPS
        pass
...
if __name__ == "__main__":
    ...
    # IMPORTANTE: No Render, use a porta da variável de ambiente
